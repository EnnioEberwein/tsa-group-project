...
The ridge paths illustrate how each standardized coefficient shrinks toward zero as $\lambda$ grows, although Rating and Income also increase partially before asymptotically approaching zero. On the left side of the plot, the coefficients correspond closely to the OLS estimates. However, this graph only depicts the coefficients based on a full training set. Thus, we can not draw any conclusions about the optimal value of $\lambda$ and the coefficients. To make this possible, we would need to calculate the MSPE for multiple values of $\lambda$ using 10-fold cross-validation and choose the minimum MSPE.
