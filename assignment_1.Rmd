...
This report presents the submission of **Group 1** for the **Time Series Analysis and Machine Learning** course (HS-2025). There are two exercises in this assignment. **First**, we replicate both panels in *James — An Introduction to Statistical Learning with Applications in R* (Figure 6.4), which display the standardized ridge regression coefficients of the credit data set. **Second**, we explain how the two principal component lines in Figure 6.14 are derived, including their meaning and why they are orthogonal to each other.
...
We start by importing the necessary libraries used throughout the project.
...
Without standardization, the penalization term in ridge regression would depend not only on the regularization parameter $\lambda$ but also on the scale of the $j$th predictor.

```{r design-matrix}
# Define predictor matrix X and response vector Y
X <- dplyr::select(
  data, Income, Limit, Rating, Cards, Age, Education, Own, Student, 
  Married, RegionEast, RegionWest, RegionSouth
)
Y <- data$Balance
...
Note: We work with the already build in scale function instead of the equation 6.6 given in the chapter six. Hence, we also mean adjust the variables, while the book only divides by the standard deviation.
...
```{r helper-functions}
# Ridge coefficients for a chosen penalty lambda with no intercept (keep all dummies)
compute_coefficients <- function(lambda, X, y) {
  ridge_fit <- lm.ridge(y ~ as.matrix(X) - 1, lambda = lambda)
  coef(ridge_fit)
}

# OLS coefficients with no intercept (keep all dummies)
...
In this section, we examine how the estimated coefficients change as the regularization strength $\lambda$ increases. To do this, we define a sequence of 100 values for $\lambda$ that span a wide range, from very small values (close to ordinary least squares) to very large ones (strong shrinkage).

For each $\lambda$, the ridge regression is re-estimated, and the resulting coefficients are stored. This creates a set of “ridge paths” that show how each variable’s influence gradually shrinks toward zero as the penalty term $\lambda \sum_{j=1}^{p} \beta_j^2$ grows.
...
In this step, we compare the $\ell_2$ norm of the ridge coefficients to the $\ell_2$ norm of the OLS coefficients which measure the square root of the sum of the squared coefficients. As the regularization strength $\lambda$ increases, the ridge coefficients shrink, and their norm becomes smaller relative to the OLS solution.

The resulting ratio (ridge norm divided by OLS norm) provides a convenient way to express the degree of shrinkage on a standardized 0–1 scale. This ratio will later serve as a rescaled x-axis when visualizing how coefficients change with increasing regularization.
...
```{r prepare-figures, include=FALSE}
if (!dir.exists("figures")) dir.create("figures")
```

### 6.1 Lambda-on-Log-Scale Plot

This plot shows how each standardized coefficient changes as the regularization strength $\lambda$ increases, using a logarithmic x-axis. At small values of $\lambda$, the model behaves like OLS with little shrinkage, while larger values gradually pull the coefficients toward zero. The highlighted variables (Income, Limit, Rating, and Student) represent the most influential predictors, while the others are shown in gray for context. Overall, the plot illustrates how ridge regression balances model complexity and stability by progressively reducing the size of the coefficients.
...
The ridge paths illustrate how each standardized coefficient shrinks toward zero as $\lambda$ grows, although Rating and Income also increase partially before asymptotically approaching zero. On the left side of the plot, the coefficients correspond closely to the OLS estimates. However, this graph only depicts the coefficients based on a full training set. Thus, we cannot draw any conclusions about the optimal value of $\lambda$ and the coefficients. To make this possible, we would need to calculate the MSPE for multiple values of $\lambda$ using 10-fold cross-validation and choose the minimum MSPE.

### 6.2 Norm-Ratio Plot

This plot presents the same coefficient paths as before but uses the ratio of the ridge to OLS $\ell_2$ norms on the x-axis instead of $\lambda$. This ratio provides a standardized measure of shrinkage that ranges from 1 (no regularization, same as OLS) to 0 (maximum shrinkage). By expressing shrinkage as a relative measure rather than a raw penalty value, this visualization makes it easier to compare the degree of regularization across different models or datasets.
...
This plot visualizes how the standardized ridge coefficients change relative to the ratio of shrinkage (the ratio of the ridge to OLS $\ell_2$ norms). On the right-hand side of the plot, the coefficients correspond to the OLS estimates when $\lambda$ is zero. Thus, the ratio is equivalent to one. Moving from right to left, the magnitude of the $\ell_2$ norm of the ridge regression declines, which decreases the ratio from one to zero. Again, we cannot draw any conclusions about the optimal value of $\lambda$ and the coefficients.

\newpage

# Exercise 2 - Principal Component Analysis

The next four paragraphs of Exercise 2 will discuss the following three questions in detail:

1. Explain in detail how the two lines in Figure 6.14 are derived.
2. Why are they orthogonal to each other?
3. What do they mean?
...
The two predictors population size ($X_1=\text{pop}$) and ad spending ($X_2=\text{ad}$) for 100 different cities are shown as purple circles. The green solid line indicates the first principal component $PC_1$, and the blue dashed line indicates the second principal component $PC_2$ derived from the data.

Principal-Component-Analysis (PCA) is a dimension reduction technique to reduce the predictor space $P=\{X_1,X_2,\ldots,X_p\}$ to $M$ principal components (PC), denoted as $Z_1, Z_2, \ldots, Z_M$ which are linear combinations of the original predictors $X_1, X_2, \ldots, X_p$. The overarching topic of this exercise is to understand how PCA uses the variables pop and ad to constructs those components.

## 2. Derivation of $PC_1$

In our case, we can, at most, derive two principal components as we have two variables. Our PCs take the form $PC_i = Z_i = a_i\cdot(\text{pop} - \overline{\text{pop}}) + b_i\cdot(\text{ad} - \overline{\text{ad}})$ with $a_i, b_i$ being the *principal component loadings*.

To derive the first principal component, we want to find a **linear combination of $\text{pop}$ and $\text{ad}$** that captures the **maximum variance** of those input variables. This is equivalent to minimizing the sum of the squared perpendicular distances between each data point and the line constructed by the linear combination, i.e. the principal component.

To proceed, it is vital to restrict $a$ and $b$ because they would otherwise blow up the variance arbitrarily. Therefore, we can only consider linear combination of the form $a^2 + b^2 = 1$.

We are now maximizing this variance using the Lagrange method:
$$\max_{\text{pop},\text{ad}}\ \text{Var}(a\cdot(\text{pop} - \overline{\text{pop}}) + b\cdot(\text{ad} - \overline{\text{ad}})) \quad \text{s.t.} \quad a^2 + b^2 = 1$$
...
## 3. Orthogonality and derivation of $PC_2$
...
With the previous calculations, we found the linear combinations of predictors that explain the highest variance in the predictors. 

The first principal component, depicted as the green line in figure 6.14 is the component that captures the most variance in the scatter plot of $X_1$ and $X_2$. The green line shows the direction in which the data varies the most as it represents the main axis of spread. Projecting the data points onto the green line provides the best one-dimensional representation of the two-dimensional data. In doing so, we are geometrically minimizing or compressing the dimensionality from two to one dimensions.

The blue line, on the other hand, indicates the direction of the remaining variation after removing the part that has already been explained by the first principal component $PC_1$. As per the definition of the uncorrelated relationship of $PC_1$ to $PC_2$, their vectors must be orthogonal to each other, with $PC_2$ representing the direction of second highest variation in the data.

```{r, echo=FALSE, out.width='100%', fig.align='center'}
knitr::include_graphics("figures/figure_6.15.jpg")
```
